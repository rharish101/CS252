<?php
  ...
  {
    $query = "
      SELECT departments.dept_name, COUNT(dept_emp.emp_no) AS count
      FROM departments
  ...
  function get_gender_ratio($conn, $dept_name)
  {
    ...
    $query = "
      SELECT employees.gender, COUNT(employees.emp_no) AS count
      FROM employees
  ...
  function get_pay_ratio($conn, $dept_name)
  {
    ...
    $query1 = "
      CREATE TABLE IF NOT EXISTS mysalaries
      (PRIMARY KEY(emp_no))
    ...
    $query3 = "
      SELECT title, COALESCE(M, 0) AS M, COALESCE(F, 0) AS F, cnt
      FROM
      (
  ...
  function get_tenure_ordered($conn, $dept_name)
  {
    ...
    $query = "
      SELECT employees.*, dept_emp.*, departments.dept_name
      FROM employees
      INNER JOIN dept_emp ON employees.emp_no = dept_emp.emp_no
      INNER JOIN departments ON dept_emp.dept_no = departments.dept_no
      WHERE departments.dept_name = '". $dept_name . "'
